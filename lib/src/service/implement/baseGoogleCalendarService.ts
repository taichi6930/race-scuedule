...
export abstract class BaseGoogleCalendarService<R extends RaceEntity>
    implements ICalendarService<R>
{
    protected readonly credentials: JWT;
    ...
    async upsertEvents(raceEntityList: RaceEntity[]): Promise<void> {
        await Promise.all(
            raceEntityList.map(async (raceEntity) => {
                // イベントIDを生成
                const eventId = this.generateEventId(raceEntity);
                try {
                    // イベントを取得
     ...
     * @returns
     */
    protected abstract generateEventId(raceEntity: RaceEntity): string;
     ...
     * @returns
     */
    protected abstract translateToCalendarEvent(
        raceEntity: RaceEntity,
