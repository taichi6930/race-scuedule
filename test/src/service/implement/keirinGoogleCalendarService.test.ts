...
import { KeirinGoogleCalendarService } from '../../../../lib/src/service/implement/keirinGoogleCalendarService';
...
/* eslint-disable */
describe('KeirinGoogleCalendarService', () => {
    const googleCalendarServiceRecord = new KeirinGoogleCalendarService(
        'testKeirinCalendarId',
    );

    const calendarDataListRecord = [baseKeirinCalendarData];

    const calendarDataListFromGoogleCalendarRecord = [
        baseKeirinCalendarDataFromGoogleCalendar,
    ];

    const raceEntityRecord = [baseKeirinRaceEntity];
    ...
    let googleCalendarService: KeirinGoogleCalendarService;
    ...
    describe(`deleteEvents`, () => {
        it('イベントが存在する場合、削除処理が行われること', async () => {
            const mockCalendarData = calendarDataListRecord[0];
            ...
            expect(google.calendar('v3').events.delete).toHaveBeenCalledTimes(
                1,
            );
        ...
        it(`イベントが存在しない場合、削除処理が行われないこと`, async () => {
            // モックの `events.insert` メソッドを設定
            const eventsDeleteMock = jest.fn().mockResolvedValue({});
            google.calendar('v3').events.delete = eventsDeleteMock;
            ...
            expect(google.calendar('v3').events.delete).not.toHaveBeenCalled();
            // console.debugで確認
            expect(console.debug).toHaveBeenCalledWith(
